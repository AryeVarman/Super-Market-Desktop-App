...
public class Order implements Serializable {
    ...
    public double getDistance(Coordinate storeCoordinate){
        double distance = -1;
        if(this.customer.getCoordinate() != null) {
            distance = Math.sqrt(Math.pow(storeCoordinate.getCol() - this.customer.getCoordinate().getCol(), 2) +
                    Math.pow(storeCoordinate.getRow() - this.customer.getCoordinate().getRow(), 2));
        }
        return distance;
    }
    ...
    public String showOrderSummery() {
        ...
        for(Store store : this.orderStoresList) {
            showOrder += "\nStore " + store.showDetailsForOrderSummery(this.customer.getCoordinate());
    ...
    public String toStringFromSingleStore(Store store) {
        ...
        double itemCount = 0;
        double itemCost = 0;
        double delivery = store.getPPK() * store.getDistanceFromStore(this.getCustomer().getCoordinate());
