...
public class MainSystem {
    ...
    private Map<Integer, Customer> customersList;
    ...
    public void startCalculateFileTask(Runnable onFinish) {
        currentRunningTask = new CalculateOpenFileTask(sdmController.getProgressTaskController().getFileNameText().getText());
        sdmController.bindFileTaskToUIComponents(currentRunningTask, onFinish);

        new Thread(currentRunningTask).start();
    }

    public void startCalculateOrderTask(Runnable onFinish) {
        currentRunningTask = new CalculateSmartOrderTask();
        sdmController.bindSmartOrderTaskToUIComponents(currentRunningTask, onFinish);
    ...
    public MainSystem(SDMController sdmController) {
        coordinateMap = new CoordinateSystem(50,50);
        storesList = new HashMap<>();
        customersList = new HashMap<>();
        itemsList = new HashMap<>();
    ...
    public void loadDataFromFile(Path path) throws Exception {
        ...
        SuperDuperMarketDescriptor marketDescriptor = (SuperDuperMarketDescriptor) unmarshaller.unmarshal(file);
        ...
        errorList += parseXMLCustomerList(marketDescriptor.getSDMCustomers(), targetCustomerList, targetStoreList);
        ...
        if(errorList.equals("XML Loading error List:\n")) {
            ...
            this.customersList = targetCustomerList;
            initItemsStatistics();
    ...
    public String showAllCustomersInSystem() {
        String CustomersStr = "Customers in the system:\n\n";

        for (Customer customer: customersList.values()) {
            CustomersStr += customer.toString() + "\n\n";
        }
    ...
    private String parseXMLCustomerList(SDMCustomers sdmCustomers, Map<Integer,Customer> targetCustomerList,
                                        Map<Integer,Store> targetStoreList) {
        String errorList = "";

        for(SDMCustomer customer : sdmCustomers.getSDMCustomer()) {
            try {
                addCustomerToCustomerList(customer, targetCustomerList, targetStoreList);
            }
            catch (Exception ex) {
                errorList += ex.getMessage();
            }
    ...
    private void addStoreToStoresList(SDMStore storeToAdd, Map<Integer, Store> targetStoreList,
        ...
        if (exceptionMsg.equals("")) {
            Store newStore = new Store(storeToAdd.getId(), storeToAdd.getName(), targetStoreItemsList,
                    new Coordinate(storeToAdd.getLocation().getX(), storeToAdd.getLocation().getY()), targetStoreSaleList,
                    storeToAdd.getDeliveryPpk());

            newStore.getCoordinate().setElement(newStore);
    ...
    private void addCustomerToCustomerList(SDMCustomer customerToAdd, Map<Integer,Customer> targetCustomerList,
                                           Map<Integer,Store> targetStoreList) throws Exception {
        String exceptionMsg = "";

        if (customerToAdd.getId() <= 0) {
            exceptionMsg += "customer: " + customerToAdd.getName() + "has non positive ID\n";
        }
        if (isStringStartOrEndWithWhiteSpace(customerToAdd.getName())) {
            exceptionMsg += "customer # " + customerToAdd.getId() + " has white space in edges of name\n";
        }
        if (targetCustomerList.containsKey(customerToAdd.getId())) {
            exceptionMsg += "customer: " + customerToAdd.getName() + " and " + targetCustomerList.get(customerToAdd.getId()).getName() +
                    " have the same ID\n";
        }

        exceptionMsg += checkLocationAndReturnErrorList(customerToAdd.getLocation(), customerToAdd.getId(), targetStoreList.values(), Store.class);
        exceptionMsg += checkLocationAndReturnErrorList(customerToAdd.getLocation(), customerToAdd.getId(), targetCustomerList.values(), Customer.class);

        if (exceptionMsg.equals("")) {

            Customer newCustomer = new Customer(customerToAdd.getId(), customerToAdd.getName(),
                    new Coordinate(customerToAdd.getLocation().getX(), customerToAdd.getLocation().getY()));

            newCustomer.getCoordinate().setElement(newCustomer);

            targetCustomerList.put(newCustomer.getSerialNumber(), newCustomer);
        }
        else {
            throw new Exception(exceptionMsg);
        }
    ...
    private void setCoordinateMap() {
        ...
        for(Store store : this.storesList.values()) {
           this.coordinateMap.changeElementInCoordinate(store.getCoordinate().getCol(), store.getCoordinate().getRow(), store);
        }

        for(Customer customer : this.customersList.values()) {
            this.coordinateMap.changeElementInCoordinate(customer.getCoordinate().getCol(), customer.getCoordinate().getRow(), customer);
        }
    }
    ...
    private void updateItemsInStoreInformation(Order order) {
        ...
        for (Order.OrderItem orderItem : order.getOrderList()) {
            if(!storesOrder.containsKey(orderItem.getStore().getSerialNumber())){
                Order subOrder = new Order(order.getDate(), order.getCustomer(), order.getSerialNumber());
                storesOrder.put(orderItem.getStore().getSerialNumber(),subOrder);
            }
            storesOrder.get(orderItem.getStore().getSerialNumber()).addOrderItemToOrderList(orderItem.getStore(),orderItem.getStoreItem(),orderItem.getAmountFromItemForThisOrder(), false);
        }
    ...
    public Map<Integer, Customer> getCustomersList() { return customersList; }
    ...
    public int getStoreSerialNumber() { return storeSerialNumber; }

    public int getItemSerialNumber() { return itemSerialNumber; }
