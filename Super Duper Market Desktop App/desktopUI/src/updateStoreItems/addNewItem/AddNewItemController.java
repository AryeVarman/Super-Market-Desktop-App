package updateStoreItems.addNewItem;
...
public class AddNewItemController {
    ...
    @FXML
    void onClickCancelBtn(ActionEvent event) { sdmController.getSdmGui().showOnScreen(prevWindow,mainBorderPane); }

    @FXML
    void onClickSubmitBtn(ActionEvent event) {
        Thread thread = new Thread(this::runSubmitAddNewItem);
    ...
    private void initItemsListComboBox() {
        for (Item item : mainSystem.getItemsList().values()) {
            if(!selectedStore.getItemsList().containsKey(item.getSerialNumber())) {
                itemsComboBox.getItems().add("#" + item.getSerialNumber() + " " + item.getName());
            }
        }
    }

    private void runSubmitAddNewItem(){
        try {
            mainSystem.addItemToStore(selectedStore.getSerialNumber(),itemID,itemPrice);
            Platform.runLater(()->sdmController.getSdmGui().showMessageDialog("The new item added successfully!"));
